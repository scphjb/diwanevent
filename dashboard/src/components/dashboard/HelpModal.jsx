            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-brand-secondary/40 text-xs">{t('common.help.subtitle')}</p>
                </div>
              </div>
              ...
              {helpItems.map((item, i) => (
                <div key={i} className="p-5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all group cursor-pointer">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl bg-brand-primary/10 flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-brand-dark transition-all shadow-inner">
                      <item.icon className="w-5 h-5" />
                    </div>
                    <div className="flex-1">
                      <h3 className="font-bold text-white text-sm mb-1">{item.title}</h3>
                      <p className="text-[11px] text-white/40 mb-3">{item.desc}</p>
                      <Button variant="outline" className="h-9 px-4 text-[10px] font-bold border-brand-primary/20 text-brand-secondary hover:bg-brand-primary hover:text-brand-dark rounded-xl">
                        {item.action}
                      </Button>
                    </div>
                  </div>
