...
const DisplayControlPage = () => {
  ...
  const scenes = [
    ...
    { id: 'wall', label: t('display_control.scenes.wall', 'حائط التواصل'), icon: MessageSquare, color: 'text-emerald-500' },
    ...
    { id: 'welcome', label: t('display_control.scenes.welcome', 'شاشة الترحيب'), icon: Tv, color: 'text-emerald-400' },
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        {channels.map((ch) => (
          ...
          >
            ...
            <div className={`text-xs opacity-60 mt-1 ${selectedChannel === ch.id ? 'text-emerald-900' : 'text-emerald-400'}`}>
      ...
      <div className="bg-white/5 border border-white/10 rounded-[32px] p-8 backdrop-blur-md">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <Link 
            ...
            target="_blank"
            className="flex items-center gap-2 h-12 px-4 rounded-xl border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-colors font-bold"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {scenes.map((scene) => (
            ...
            >
              <scene.icon className={`w-16 h-16 mb-4 ${currentScene === scene.id ? 'text-emerald-400' : scene.color}`} />
              <span className={`text-2xl font-bold ${currentScene === scene.id ? 'text-white' : 'text-white/70'}`}>
                {scene.label}
              </span>
              
              {currentScene === scene.id && (
                <div className="absolute top-4 right-4 flex items-center gap-2 text-emerald-400 text-sm font-bold bg-emerald-500/20 px-3 py-1 rounded-full">
