...
const OverviewPage = () => {
  ...
  if (loading) return (
    <DashboardLayout>
      <div className="p-20 text-center flex flex-col items-center gap-6 justify-center min-h-[60vh]">
        ...
        <div className="text-emerald-400 font-bold animate-pulse">{t('dashboard.loading_data', 'جاري تحضير لوحة التحكم...')}</div>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout>
      <div className="p-10 flex flex-col items-center justify-center min-h-[70vh] text-center">
        ...
        >
          ...
          <div className="w-24 h-24 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-emerald-500/20">
            <Calendar className="w-12 h-12 text-white" />
          </div>
          
          <h1 className="text-4xl font-black text-white mb-4">
            أهلاً بك في <span className="text-emerald-500 font-black">ديوان</span>
          </h1>
          <p className="text-emerald-400/60 text-lg mb-10 leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button 
              ...
              className="px-10 py-4 bg-emerald-500 text-emerald-950 font-black rounded-2xl hover:bg-emerald-400 hover:scale-105 transition-all shadow-xl shadow-emerald-900/40"
            >
              إنشاء فعالية جديدة
            </button>
            <div className="flex items-center gap-2 px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-emerald-400/40 font-bold">
  ...
  if (!eventId) return (
    <DashboardLayout>
      <div className="p-20 text-center flex flex-col items-center gap-6 justify-center min-h-[60vh]">
        <div className="w-16 h-16 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-emerald-400">
          <Calendar className="w-8 h-8" />
        </div>
        <div className="text-emerald-400/60 font-bold">{t('dashboard.select_event', 'يرجى اختيار فعالية من القائمة بالأعلى لعرض الإحصائيات.')}</div>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout activePath="/dashboard">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white/5 border border-white/10 rounded-[40px] p-10 backdrop-blur-md">
          <div className="flex items-center justify-between mb-10">
            ...
            <div className="flex gap-2 items-center">
               <div className="px-3 py-1 rounded-lg bg-emerald-500/10 text-[10px] text-emerald-400 font-bold">{t('dashboard.capacity', 'السعة الإجمالية')}: {data.overview.hall_capacity}</div>
               ...
               {data.halls_occupancy?.map(h => (
                 <div key={h.name} className="px-3 py-1 rounded-lg bg-white/5 text-[10px] text-white/40 font-bold flex gap-2 items-center">
                   <div className={cn("w-1.5 h-1.5 rounded-full", h.rate > 90 ? "bg-red-500" : h.rate > 70 ? "bg-amber-500" : "bg-emerald-500")} />
        ...
        <div className="space-y-6">
          <div className={cn(
            "rounded-[40px] p-8 text-emerald-950 flex flex-col justify-between h-[200px] relative overflow-hidden shadow-2xl transition-all duration-500",
            data.overview.occupancy_rate > 80 ? "bg-gradient-to-br from-amber-500 to-amber-700 shadow-amber-900/40" : "bg-gradient-to-br from-emerald-500 to-emerald-700 shadow-emerald-900/40"
          )}>
          ...
          <div className="bg-white/5 border border-white/10 rounded-[40px] p-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                <div className={cn("w-2 h-2 rounded-full", data.overview.checked_in > 0 ? "bg-emerald-500" : "bg-amber-500")} />
                <div className="text-xs text-white/70">
