...
const NetworkingHub = ({ eventId, participant }) => {
  ...
  const showToast = (message, type = 'info') => {
    // Simple toast for MVP, you might want to use a proper toast library like react-hot-toast
  ...
  useEffect(() => {
    ...
    // Using standard wss/ws protocol 
...
const DirectoryTab = ({ eventId, myId }) => {
  ...
  useEffect(() => {
    ...
    // Fetch full directory once to extract dynamic tags
  ...
  return (
    <div style={{ padding:16 }} dir="rtl">
      ...
      {/* Visibility Toggle */}
      <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:12, padding:14, marginBottom:16, display:'flex', alignItems:'center', justifyContent:'space-between', border:'1px solid rgba(255,255,255,0.06)' }}>
        <div>
          <div style={{ color:'#F0F4F2', fontSize:14, fontWeight:'bold', display:'flex', alignItems:'center', gap:6 }}>
             الظهور الميداني
          </div>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, overflowX:'auto', paddingBottom:4 }}>
        ...
        {availableTags.map(spec => (
          <button key={spec}
            ...
            style={{ padding:'6px 14px', borderRadius:20, border: selectedSpecialty === spec ? '1px solid #D4AF37' : '1px solid rgba(255,255,255,0.15)', fontFamily:'Cairo', fontSize:11, cursor:'pointer', whiteSpace:'nowrap', transition:'all 0.2s',
              background: selectedSpecialty === spec ? 'rgba(212,175,55,0.2)' : 'rgba(255,255,255,0.03)',
              color: selectedSpecialty === spec ? '#D4AF37' : 'rgba(255,255,255,0.6)',
            }}
          >#{spec}</button>
        ))}
      </div>
  ...
  if (activeChat) return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }} dir="rtl">
      ...
      <div style={{ padding:'12px 16px', borderBottom:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', gap:10 }}>
        <button onClick={() => setActiveChat(null)} style={{ background:'none', border:'none', color:'#D4AF37', cursor:'pointer', fontSize: 18 }}>←</button>
...
const RecommendationCard = ({ participant: p, onConnect }) => (
  <div style={{ minWidth:150, background:'rgba(255,255,255,0.04)', borderRadius:12, padding:12, border:'1px solid rgba(212,175,55,0.15)', textAlign:'center' }}>
    ...
    {p.connection_status === 'pending' ? (
      <button disabled style={{ background:'rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.5)', border:'none', borderRadius:8, padding:'5px 12px', fontFamily:'Cairo', fontSize:11 }}>
        طلب مرسل
      </button>
    ) : (
