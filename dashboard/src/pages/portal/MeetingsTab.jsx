  ...
  return (
    <div style={{ padding: 16 }} dir="rtl">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h3 style={{ color: isLightTheme ? '#475569' : 'rgba(255,255,255,0.6)', fontSize: 14, margin: 0 }}>جدول الاجتماعات</h3>
        <button 
          onClick={() => setIsProposing(!isProposing)}
          style={{ 
            background: isLightTheme ? '#D97706' : '#D4AF37', 
            color: '#FFFFFF', 
            border: 'none', borderRadius: 20, padding: '6px 14px', fontFamily: 'Cairo', fontSize: 12, fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 
          }}
        >
      ...
      {isProposing && (
        <form onSubmit={handlePropose} style={{ 
          background: isLightTheme ? '#FFFFFF' : 'rgba(255,255,255,0.04)', 
          borderRadius: 12, padding: 16, marginBottom: 24, 
          border: isLightTheme ? '1px solid rgba(217,119,6,0.3)' : '1px solid rgba(212,175,55,0.3)',
          boxShadow: isLightTheme ? '0 4px 12px rgba(15,23,42,0.03)' : 'none'
        }}>
          <h4 style={{ margin: '0 0 12px 0', color: isLightTheme ? '#D97706' : '#D4AF37', fontSize: 14 }}>تفاصيل الاجتماع</h4>
          
          <select required value={selectedConnection} onChange={e => setSelectedConnection(e.target.value)} style={{ 
            ...
            borderRadius: 8, color: isLightTheme ? '#0F172A' : '#fff', marginBottom: 10, fontFamily: 'Cairo' 
          }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
            <input required type="date" value={proposedDate} onChange={e => setProposedDate(e.target.value)} style={{ 
              flex: 1, padding: 10, 
              background: isLightTheme ? '#FFFFFF' : 'rgba(0,0,0,0.2)', 
              border: isLightTheme ? '1px solid rgba(15,23,42,0.15)' : '1px solid rgba(255,255,255,0.1)', 
              borderRadius: 8, color: isLightTheme ? '#0F172A' : '#fff', fontFamily: 'Cairo' 
            }} />
            <input required type="time" value={proposedTime} onChange={e => setProposedTime(e.target.value)} style={{ 
              flex: 1, padding: 10, 
              background: isLightTheme ? '#FFFFFF' : 'rgba(0,0,0,0.2)', 
              border: isLightTheme ? '1px solid rgba(15,23,42,0.15)' : '1px solid rgba(255,255,255,0.1)', 
              borderRadius: 8, color: isLightTheme ? '#0F172A' : '#fff', fontFamily: 'Cairo' 
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
            ...
            <input placeholder="المكان (مثال: ردهة الفندق)" value={locationNote} onChange={e => setLocationNote(e.target.value)} style={{ 
              flex: 2, padding: 10, 
              background: isLightTheme ? '#FFFFFF' : 'rgba(0,0,0,0.2)', 
              border: isLightTheme ? '1px solid rgba(15,23,42,0.15)' : '1px solid rgba(255,255,255,0.1)', 
              borderRadius: 8, color: isLightTheme ? '#0F172A' : '#fff', fontFamily: 'Cairo' 
          ...
          <textarea placeholder="موضوع الاجتماع / الأجندة" value={agenda} onChange={e => setAgenda(e.target.value)} style={{ 
            width: '100%', padding: 10, 
            background: isLightTheme ? '#FFFFFF' : 'rgba(0,0,0,0.2)', 
            border: isLightTheme ? '1px solid rgba(15,23,42,0.15)' : '1px solid rgba(255,255,255,0.1)', 
            borderRadius: 8, color: isLightTheme ? '#0F172A' : '#fff', fontFamily: 'Cairo', marginBottom: 12, minHeight: 60, boxSizing: 'border-box' 
          }} />
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button type="submit" style={{ 
              ...
              background: isLightTheme ? '#D97706' : '#D4AF37', 
              color: '#FFFFFF', 
              border: 'none', borderRadius: 8, padding: 10, fontWeight: 'bold', cursor: 'pointer', fontFamily: 'Cairo' 
            }}>إرسال الطلب</button>
            <button type="button" onClick={() => setIsProposing(false)} style={{ 
              background: 'transparent', 
              color: isLightTheme ? '#64748B' : '#fff', 
              border: isLightTheme ? '1px solid rgba(15,23,42,0.15)' : '1px solid rgba(255,255,255,0.2)', 
              borderRadius: 8, padding: 10, cursor: 'pointer', fontFamily: 'Cairo' 
            }}>إلغاء</button>
          </div>
        </form>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {pendingRequests.length > 0 && (
            <div>
              <h4 style={{ color: isLightTheme ? '#D97706' : '#D4AF37', margin: '0 0 10px 0', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}><Clock size={14}/> طلبات واردة بانتظار ردك</h4>
          ...
          {myProposals.length > 0 && (
            <div>
              <h4 style={{ color: isLightTheme ? '#64748B' : 'rgba(255,255,255,0.4)', margin: '0 0 10px 0', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}><Clock size={14}/> بانتظار رد الطرف الآخر</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {myProposals.map(m => (
...
const MeetingCard = ({ meeting, formatter, onRespond, isConfirmed, isWaiting }) => {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, 
      padding: 14,
      border: `1px solid ${isConfirmed ? 'rgba(34, 197, 94, 0.3)' : (isLightTheme ? 'rgba(15,23,42,0.06)' : 'rgba(255,255,255,0.06)')}`,
      borderRight: isConfirmed ? '3px solid #22c55e' : 'none',
      boxShadow: isLightTheme ? '0 4px 12px rgba(15,23,42,0.02)' : 'none'
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        <div>
          <div style={{ color: isLightTheme ? '#475569' : 'rgba(255,255,255,0.6)', fontSize: 11, marginBottom: 4 }}>اجتماع مع:</div>
          <div style={{ color: isLightTheme ? '#0F172A' : '#F0F4F2', fontWeight: 'bold', fontSize: 14 }}>{meeting.with.full_name}</div>
          <div style={{ color: isLightTheme ? '#475569' : 'rgba(255,255,255,0.4)', fontSize: 11 }}>{meeting.with.council}</div>
        </div>
        <div style={{ 
          textAlign: 'left', 
          background: isLightTheme ? 'rgba(217,119,6,0.08)' : 'rgba(0,0,0,0.2)', 
          padding: '6px 10px', borderRadius: 8 
        }}>
          <div style={{ color: isLightTheme ? '#D97706' : '#D4AF37', fontWeight: 'bold', fontSize: 13 }}>{time}</div>
          <div style={{ color: isLightTheme ? '#B45309' : 'rgba(255,255,255,0.6)', fontSize: 11 }}>{date}</div>
        </div>
      </div>

      <div style={{ 
        display: 'flex', flexWrap: 'wrap', gap: 12, fontSize: 11, 
        color: isLightTheme ? '#334155' : 'rgba(255,255,255,0.7)', marginBottom: 12, 
        background: isLightTheme ? 'rgba(15,23,42,0.02)' : 'rgba(255,255,255,0.02)', padding: 8, borderRadius: 6 
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={12} color={isLightTheme ? '#D97706' : '#D4AF37'}/> {meeting.duration_minutes} دقيقة</div>
        {meeting.location_note && <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><MapPin size={12} color={isLightTheme ? '#D97706' : '#D4AF37'}/> {meeting.location_note}</div>}
        {meeting.agenda && <div style={{ width: '100%', marginTop: 4, color: isLightTheme ? '#475569' : 'rgba(255,255,255,0.5)' }}>📝 {meeting.agenda}</div>}
      </div>
      ...
      {isWaiting && (
         <div style={{ color: isLightTheme ? '#64748B' : 'rgba(255,255,255,0.4)', fontSize: 11, textAlign: 'center' }}>تم الإرسال... بانتظار التأكيد</div>
      )}
    </div>
  );
};
