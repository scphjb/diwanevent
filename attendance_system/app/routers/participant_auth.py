...
async def request_otp(body: OTPRequest, db: Session = Depends(get_db)):
    ...
    participant = db.query(Participant).filter(
        Participant.order_num == body.order_num
    ).first()
    ...
    recent_otp = db.query(ParticipantOTP).filter(
    ...
    # 4. إلغاء OTP سابق
    db.query(ParticipantOTP).filter(
        ParticipantOTP.participant_id == participant.id,
        ParticipantOTP.is_used == False,
    ).update({"is_used": True})
    ...
    # 6. جلب اسم الفعالية
    event_name = "الفعالية"
    if hasattr(participant, 'event') and participant.event:
        event_name = participant.event.event_name or event_name
...
async def verify_otp(body: OTPVerify, db: Session = Depends(get_db)):
    ...
    otp_record = db.query(ParticipantOTP).filter(
        ParticipantOTP.participant_id == participant.id,
        ParticipantOTP.is_used == False,
        ParticipantOTP.email == body.email.lower().strip(),
    ).order_by(ParticipantOTP.created_at.desc()).first()
...
@router.get("/me")
def get_participant_me(participant: Participant = Depends(get_current_participant)):
